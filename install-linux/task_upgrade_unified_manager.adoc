...
+
Wenn fehlende Pakete erkannt werden, führen Sie die unter genannten Schritte link:../install-linux/task_upgrade_unified_manager.html#additional-steps-to-perform-for-missing-packages["Weitere Schritte, die bei fehlenden Paketen ausgeführt werden müssen"]aus. Wenn keine Pakete vorhanden sind, fahren Sie mit den nächsten Schritten fort.
