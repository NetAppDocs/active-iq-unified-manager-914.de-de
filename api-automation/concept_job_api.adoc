...
[role="lead"]
Sie können die API unter der `management-server` Kategorie verwenden `jobs`, um die Ausführungsdetails asynchroner Vorgänge anzuzeigen. Über die `system` API unter der `management-server` Kategorie können Sie Details zur Instanz in Ihrer Active IQ Unified Manager-Umgebung anzeigen.
