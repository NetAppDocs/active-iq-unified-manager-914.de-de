...
[NOTE]
====
Das „`WFA Paket für das Management von Clustered Data ONTAP`“ ist in WFA 5.1 enthalten, sodass Sie dieses Paket nicht mehr aus dem NetApp Storage Automation Store herunterladen und es je nach Anforderung separat auf Ihrem WFA Server installieren müssen.  https://automationstore.netapp.com/pack-list.shtml["WFA Pack zum Management von ONTAP"]
