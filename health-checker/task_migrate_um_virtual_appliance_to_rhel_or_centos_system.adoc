...
+
** Sie müssen Unified Manager auf einem Linux-Server gemäß den Anweisungen in installiert habenlink:../install-linux/concept_install_unified_manager_on_rhel_or_centos.html["Installation von Unified Manager auf Linux Systemen"].
...
. Melden Sie sich als Root-Benutzer beim Linux-Host an, auf dem Unified Manager installiert ist, und kopieren Sie die Backup-Dateien unter Verwendung von SCP aus /Support auf der virtuellen Appliance.`root@<rhel_server>:/# scp -r admin@<vapp_server_ip_address>:/support/* .`
+
